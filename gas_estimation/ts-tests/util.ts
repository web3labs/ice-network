...
export const DISPLAY_LOG = process.env.ICE_NODE_LOG || false;
export const ICE_NODE_LOG = process.env.ICE_NODE_LOG || "info";
export const ICE_NODE_BUILD = process.env.ICE_NODE_BUILD || "release";

export const BINARY_PATH = `/home/user/web3labs/ice-release/ice-substrate/target/${ICE_NODE_BUILD}/ice-node`;
...
export async function startIceNode(provider?: string): Promise<{ web3: Web3; binary: ChildProcess, ethersjs: ethers.providers.JsonRpcProvider }> {
	var web3;
	...
	await new Promise((resolve) => {
		const timer = setTimeout(() => {
			console.error(`\x1b[31m Failed to start ICE Node.\x1b[0m`);
